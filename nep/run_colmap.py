...
def run_sfm(image_dir, project_dir, same_camera=False, colmap_path='$HOME/code/colmap/build/src/exe/colmap', no_dense=False):
    ...
    for k, img_fn in enumerate(img_fns):
        ...
        if same_camera:
            if k == 0:
                global_cam_id = db.add_camera(CAMERA_MODEL_NAMES['SIMPLE_PINHOLE'].model_id,
    ...
    if no_dense:
        cmd = [colmap_path, 'model_converter',
               ...
               '--output_path',  f'{project_dir}/sparse/0/points3D.ply',
               '--output_type',  'PLY',
               ]
        print(' '.join(cmd))
        subprocess.run(cmd, check=True)
...
def main():
    ...
    parser.add_argument('--same_camera', action='store_true',
                        default=False, dest='same_camera')
    parser.add_argument('--no-dense', action='store_true', default=False)
    args = parser.parse_args()
    image_dir = f'{args.project_dir}/images'
    run_sfm(image_dir, args.project_dir, args.same_camera,
            colmap_path=args.colmap, no_dense=args.no_dense)
