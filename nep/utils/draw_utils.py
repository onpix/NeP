...
def vis_directions(tensor_list, filepath):
    """
    plot ray directoins
    """
    fig = go.Figure()
    ...
    args_list = [
        {"scatter_color": "black", "line_color": "gray"},
        {"scatter_color": "red", "line_color": "pink"},
        {"scatter_color": "blue", "line_color": "lightskyblue"},
        {"scatter_color": "green", "line_color": "lightseagreen"},
    ]
    ...
    for tensor_idx, tensor in enumerate(tensor_list):
        scatter = go.Scatter3d(
            ...
            marker=dict(
                size=5,
                color=args_list[tensor_idx]["scatter_color"],
            ),
            opacity=0.7,
        )
        fig.add_trace(scatter)

        # Create lines from [0, 0, 0] to each point
        for i in range(tensor.shape[0]):
            line = go.Scatter3d(
                ...
                z=[0, tensor[i, 2].item()],
                mode="lines",
                line=dict(color=args_list[tensor_idx]["line_color"], width=2),
                opacity=0.4,
            )
            fig.add_trace(line)
        ...
        fig.update_layout(
            scene=dict(
                xaxis=dict(showbackground=False, zerolinecolor="black"),
                yaxis=dict(showbackground=False, zerolinecolor="black"),
                zaxis=dict(showbackground=False, zerolinecolor="black"),
            ),
            width=700,
...
def add_text_to_image(tensor_image, text, position=(20, 20), color="white"):
    """
    Add text to a PyTorch tensor image at the given position.

    Args:
    tensor_image (torch.Tensor): an image in the form of a PyTorch tensor with shape (3, h, w)
    text (str): the text to be added
    position (tuple): the position where to add the text (default: (0,0))

    Returns:
    PIL.Image: the resulting image with added text
    """
    assert tensor_image.ndim == 3 and (tensor_image.shape[0] == 3 or tensor_image.shape[-1] == 3)
    np_image = tensor_image.numpy()
    if np_image.shape[0] == 3:
        # Convert the tensor to a NumPy array and transpose dimensions for PIL,
        # which expects images in the format (h, w, 3)
    ...
    font = ImageFont.truetype("./files/Roboto-Medium.ttf", size=40)

    # Draw the text in black at each offset position
    draw.text(position, text, fill=color, font=font)
    np_image_with_text = np.array(pil_image)
    tensor_image_with_text = torch.from_numpy(np_image_with_text).float() / 255.0

    if tensor_image.shape[0] == 3:
        # Convert back to (3, h, w) format
        tensor_image_with_text = tensor_image_with_text.permute(2, 0, 1).permute(2, 0, 1)

    return tensor_image_with_text
