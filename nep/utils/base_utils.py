...
def check_merge(base_dict, other_dict, prefix="", raise_err=True):
    if base_dict == None:
        # other_dict is a dict and base_dict is None.
        return

    for key in other_dict:
        if key not in base_dict:
            msg = f"Key {key} in the second dictionary doesn't exist in the first dictionary"
            if raise_err:
                raise KeyError(msg)
            else:
                print("Warn:", msg)

        if isinstance(other_dict[key], Mapping):
            check_merge(
                base_dict[key], other_dict[key], prefix=f"{prefix}.{key}", raise_err=raise_err
            )

        elif base_dict[key] != other_dict[key]:
            print(f"Config override: {prefix}.{key}: {base_dict[key]} -> {other_dict[key]}")


def merge_config(base_dict, other_dict, ignore_check=False):
    if not ignore_check:
        print("Checking config merge...")
        check_merge(base_dict, other_dict, raise_err=True)
    else:
        print("Skip checking config merge...")

    base_dict = omegaconf.OmegaConf.create(base_dict)
    other_dict = omegaconf.OmegaConf.create(other_dict)
    return omegaconf.OmegaConf.merge(base_dict, other_dict)


def extrac_sub_state_dict(param, prefix):
    sub_dict = OrderedDict()
    for key, value in param.items():
        if key.startswith(prefix):
            sub_dict[key.replace(f"{prefix}.", "", 1)] = value
    return sub_dict
...
def normalize_image(img, mask=None):
    if mask is not None:
        img[np.logical_not(mask.astype(np.bool_))] = 127
...
def get_img_patch(img, pt, size):
    ...
    patch[ymin - y + size_h : ymax - y + size_h, xmin - x + size_w : xmax - x + size_w] = img[
        ymin:ymax, xmin:xmax
    ]
    return patch
...
def compute_F(K1, K2, R, t):
    ...
    """
    A = K1 @ R.T @ t  # [3,1]
    C = np.asarray([[0, -A[2, 0], A[1, 0]], [A[2, 0], 0, -A[0, 0]], [-A[1, 0], A[0, 0], 0]])
...
def dict_merge(base_dict, merge_dict, parent_key="", warn_new_key=False):
    for k, v in merge_dict.items():
        if (
            k in base_dict
            and isinstance(base_dict[k], Mapping)
            ...
            dict_merge(
                ...
                warn_new_key=warn_new_key,
            )
        else:
            if k not in base_dict and warn_new_key:
                ...
                # print(
                #     f'Warn: Key "{parent_key}/{k}" from merging config is not in the base config.'
                # )

            base_dict[k] = merge_dict[k]

            # if merge_path:
            #     print(f'* Merge new item from {merge_path.name}: \n{k}: {base_dict[k]}')
...
def load_cfg(path):
    ...
    if "from" in main_cfg:
        base_cfg_path = path.parent / Path(main_cfg.pop("from"))
        print(f"> Parsing {path}, load father config at: {base_cfg_path}")
        base_cfg = load_cfg(base_cfg_path)

        dict_merge(base_cfg, main_cfg, warn_new_key=True)
        return base_cfg
    else:
        return main_cfg
...
def ransac_pnp(
    ...
    points_3d = np.ascontiguousarray(points_3d.astype(np.float64))
    camera_matrix = camera_matrix.astype(np.float64)
    state, R_exp, t, inliers = cv2.solvePnPRansac(
        points_3d,
        points_2d,
        camera_matrix,
        dist_coeffs,
        flags=method,
        iterationsCount=iter_num,
        reprojectionError=rep_error,
        confidence=0.999,
    )
    mask = np.zeros([points_3d.shape[0]], np.bool_)
    if state:
        R, _ = cv2.Rodrigues(R_exp)
        mask[inliers[:, 0]] = True
...
def save_depth(fn, depth, max_val=1000):
    ...
    with open(fn, "wb") as f:
        writer = png.Writer(
            width=depth.shape[1], height=depth.shape[0], bitdepth=16, greyscale=True
        )
...
def generate_coords(h, w, device):
    coords = torch.stack(
        torch.meshgrid(torch.arange(h, device=device), torch.arange(w, device=device)),
        -1,
    )
    return coords[..., (1, 0)]
...
def tan_2theta(tan_theta):
    return 2 * tan_theta / (1 - tan_theta**2)


def roughness2area(roughness, max_tan=0.26794919243, roughness_transform=lambda x: x):
    # equation from: https://agraphicsguy.wordpress.com/2015/11/01/sampling-microfacet-brdf/
    roughness = roughness_transform(roughness)
    tan_theta = roughness * 3
    tan_cone_angle = tan_2theta(tan_theta)
    tan_cone_angle = torch.clamp(tan_cone_angle, 0, max_tan)
    return tan_cone_angle**2 * torch.pi

    # cos_theta = torch.sqrt((1.0 - epsilon + 1e-6) / (1.0 + (roughness**2 - 1.0) * epsilon + 1e-6) + 1e-6) # pn,sn,1
    # theta = torch.arccos(cos_theta) / 8
    # cone_half_deg = 2 * theta
    # tan_theta = torch.tan(cone_half_deg)

    # this theta is n · m, which is 1/2 cone angle
    # cone angle is the half angle at the cone vertex.
    # tan(cone_angle) is exectly the same as the radius of the pixel

    # 0.26794919243 = tan 15 deg
    # 3 = math.sqrt(0.9 / 0.1), whcih means epsilon = 0.9
    # tan_theta = roughness * 3
    # tan_theta = torch.clamp(tan_theta, 0, max_tan_theta)


# def merge_ns_tensor_dataclass(base_obj, merging_obj, **args):
#     for k, v in vars(base_obj).items():
#         if isinstance(v, torch.Tensor):
#             setattr(base_obj, k, torch.cat([getattr(base_obj, k), getattr(merging_obj, k)], **args))

# def merge_ns_ray_samples(base_samples, merging_samples):
#     merge_ns_tensor_dataclass(base_samples.frustums, merging_samples.frustums, dim=1)
#     merge_ns_tensor_dataclass(base_samples, merging_samples, dim=1)


def check_memory_usage(name, device="cuda:0"):
    allocated = torch.cuda.memory_allocated(device) / (1024**2)  # in MBs
    reserved = torch.cuda.memory_reserved(device) / (1024**2)  # in MBs
    print(f"[ {name} ] - Memory Allocated: {allocated:.2f} MB, Memory Reserved: {reserved:.2f} MB")


def normalize_using_percentile(image, percentile=0.95):
    # Compute the value at the given percentile
    k = int(percentile * image.nelement())
    percentile_value, _ = torch.kthvalue(image.flatten(), k)
    normed_image = image / percentile_value
    normed_image = torch.clamp(normed_image, 0, 1)
    return normed_image
