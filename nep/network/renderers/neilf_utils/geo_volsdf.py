            ...
            with torch.enable_grad():
                ...
                if grad_order >= 2:
                    if self.hess_type == "analytic":
                        hess = torch.stack(
                            [compute_gradient(x, grad[:, i : i + 1]) for i in range(grad.shape[1])],
                            dim=1,
                        )
                    elif self.hess_type == "numeric":
                        jitter_range = (2 / 4096, 2 / 1024)  # NOTE hard code
                        jitter_dir = torch.eye(3, dtype=x.dtype, device=x.device).unsqueeze(
                            0
                        )  # 133
                        jitter_step = torch.rand(
                            (x.shape[0], 1, 1), dtype=x.dtype, device=x.device
                        )  # m11
                        jitter_step = (
                            jitter_step * (jitter_range[1] - jitter_range[0]) + jitter_range[0]
                        )  # TODO pos+neg?
                        jitter_points_all = x.unsqueeze(1) + jitter_dir * jitter_step  # m33
                        jitter_points_sample = torch.randint(
                            0,
                            3,
                            (jitter_points_all.shape[0],),
                            dtype=torch.long,
                            device=jitter_points_all.device,
                        )
                        jitter_points = jitter_points_all[
                            torch.arange(
                                jitter_points_all.shape[0],
                                dtype=torch.long,
                                device=jitter_points_all.device,
                            ),
                            jitter_points_sample,
                        ]
...
class Geometry(nn.Module):
    def __init__(self, config_model, phase, num_reg_samples=None, calc_hess=None):
        ...
        self.rendering_network = RenderingNetwork(
            self.feature_vector_size, **config_model.get("rendering_network")
        )

        self.density = LaplaceDensity(**config_model.get("density"))
        self.ray_sampler = ErrorBoundSampler(
            self.scene_bounding_sphere,
            inverse_sphere_bg=(self.bkgd_type == "nerfpp"),
            **config_model.get("ray_sampler")
        )

        config_bkgd = config_model["background"]
        if self.bkgd_type == "uniform":
            self.bg_color = (
                torch.tensor(config_bkgd.get("bg_color", [1.0, 1.0, 1.0])).float().cuda()
            )
            ...
            self.bg_rendering_network = RenderingNetwork(
                self.bg_feature_vector_size, **config_bkgd.get("rendering_network")
            )
    ...
    def forward(self, ray_bundle):
        ...
        ray_dirs, cam_loc = ray_bundle.directions, ray_bundle.origins

        num_pixels, _ = ray_dirs.shape

        z_vals, z_samples_eik = self.ray_sampler.get_z_vals(
            ray_dirs, cam_loc, self.density, self.implicit_network, take_sphere_intersection=True
        )
        z_max = None
        ...
        used_normals = (
            unit_pred_normals_flat if self.rendering_network.mode == "refnerf" else gradients
        )
        ...
        if self.bkgd_type == "uniform":
            ...
            bg_sdf, bg_feature_vectors, bg_aux = self.bg_implicit_network.get_outputs(
                bg_points_flat
            )
        ...
        if self.bkgd_type == "none":
            ...
            # can not be all False
            # if not render_masks.any():
            #     import ipdb; ipdb.set_trace()
            # assert render_masks.any()
        ...
        if self.training and self.phase in ["geo", "joint"]:
            ...
            object_rand_points = (
                torch.empty(self.num_reg_samples, 3)
                .uniform_(-self.object_bounding_sphere, self.object_bounding_sphere)
                .cuda()
            )
            scene_rand_points = (
                torch.empty(num_pixels, 3)
            ...
            if self.calc_hess:
                rand_sdfs, rand_grad, rand_hess = self.implicit_network.get_sdf_vals_grad(
                    rand_points, clamp=False, grad_order=2
                )
                aux_output["hess_theta"] = rand_hess
            else:
                rand_sdfs, rand_grad = self.implicit_network.get_sdf_vals_grad(
                    rand_points, clamp=False, grad_order=1
                )

            # add some of the near surface points
            near_points = (
                cam_loc.unsqueeze(1) + z_samples_eik.unsqueeze(2) * ray_dirs.unsqueeze(1)
            ).reshape(-1, 3)
            near_sdfs, near_grad = self.implicit_network.get_sdf_vals_grad(
                near_points, clamp=False, grad_order=1
            )
            ...
            #         aux_output["pcd_sdfs"],
            #         aux_output["pcd_grad"],
            #     ) = self.implicit_network.get_sdf_vals_grad(
            #         pcd_positions, clamp=False, grad_order=1
            #     )
    ...
    def trace_and_render(self, points, normals, ray_dirs, sample=None, validate_normal=False):
        ...
        if trace_total_num > trace_sample_num:
            trace_sample = torch.multinomial(
                torch.full((trace_total_num,), 1 / trace_total_num),
                trace_sample_num,
                replacement=False,
            )
        else:
        ...
        z_vals, z_samples_eik = self.ray_sampler.get_z_vals(
            ray_dirs, points, self.density, self.implicit_network, take_sphere_intersection=True
        )
        z_max = None
        if self.bkgd_type == "nerfpp":
            z_vals, z_vals_bg = z_vals
        ...
        used_normals = (
            Func.normalize(imp_aux["pred_grad"], dim=-1)
            if self.rendering_network.mode == "refnerf"
            else gradients
        )
        ...
        ray_dirs, points_flat, used_normals, dirs_flat, feature_vectors, weights = [
            apply_mask(arr)
            for arr in [ray_dirs, points_flat, used_normals, dirs_flat, feature_vectors, weights]
        ]
    ...
    def volume_rendering(self, z_vals, sdf, n_d_r=None, z_max=None):
        ...
        if not (bg_transmittance < 0.05).any():
            import ipdb; ipdb.set_trace() 
    ...
    def bg_volume_rendering(self, z_vals_bg, bg_sdf):
        ...
        bg_dists = torch.cat(
            [bg_dists, torch.tensor([1e10]).cuda().unsqueeze(0).repeat(bg_dists.shape[0], 1)], -1
        )
    ...
    def get_surface_trace(
        ...
        for i, pnts in enumerate(torch.split(points, 100000, dim=0)):
            z.append(
                self.implicit_network.get_sdf_vals(pnts, clamp=False)[:, 0].detach().cpu().numpy()
            )
...
class GeoLoss(nn.Module):
    ...
    def forward(self, model_outputs, ground_truth, progress_iter):
        ...
        if (
            "weights" in geo_outputs
            and "ndv" in geo_outputs
            and model_outputs["render_masks"].sum().item() > 0
        ):
            orientation_loss = self.get_orientation_loss(
                geo_outputs["weights"], geo_outputs["ndv"], model_outputs["render_masks"]
            )
        else:
        ...
        if (
            "weights" in geo_outputs
            and "normals_dot" in geo_outputs
            and model_outputs["render_masks"].sum().item() > 0
        ):
            pred_normal_loss = self.get_pred_normal_loss(
                geo_outputs["weights"], geo_outputs["normals_dot"], model_outputs["render_masks"]
            )
        else:
        ...
        if "pcd_sdfs" in geo_outputs and "pcd_grad" in geo_outputs:
            pcd_normals = (
                ground_truth["pcd_normals"].cuda().reshape(-1, 3)
                if "pcd_normals" in ground_truth
                else None
            )
            pcd_loss = self.get_pcd_loss(
                geo_outputs["pcd_sdfs"], geo_outputs["pcd_grad"], pcd_normals
            )
        else:
            pcd_loss = ZERO
