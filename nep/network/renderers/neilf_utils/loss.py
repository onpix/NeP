...
class NeILFLoss(nn.Module):
    ...
    def forward(self, model_outputs, ground_truth, progress_iter):
        ...
        if self.phase in ["mat", "joint"]:
            ...
            # rendered rgb
            rgb_values = model_outputs["ray_rgb"]
            rgb_loss = (
                (self.rgb_loss_func(rgb_values, rgb_gt) * masks.unsqueeze(1)).sum() / mask_sum / 3
            )
            loss_dict["loss_rgb"] = rgb_loss

            # smoothness smoothness
            # fix: remove rgb_grad here as we don't have it
            ...
            if self.mono_neilf:
                gray_weight = torch.tensor(
                    [[0.2989, 0.5870, 0.1140]],
                    dtype=trace_render_rgb.dtype,
                    device=trace_render_rgb.device,
                )
                trace_render_rgb = (trace_render_rgb * gray_weight).sum(dim=-1, keepdim=True)
                trace_nn_rgb = trace_nn_rgb[..., :1]
            trace_loss = (trace_nn_rgb - trace_render_rgb).abs().sum() / (
                trace_nn_rgb.numel() + 1e-9
            )
            loss_dict["loss_trace"] = trace_loss
            ...
            if "rgb_var" in model_outputs and progress_iter <= 1000:
                ...
                roughness_target = (1 - roughness_target.clamp(0, 1)) * (
                    tgt_max - tgt_min
                ) + tgt_min
                var_loss = (
                    (roughness - roughness_target).abs() * masks.unsqueeze(1)
                ).sum() / mask_sum
                loss_dict["loss_var"] = var_loss
