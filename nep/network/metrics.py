...
def draw_materials(data_pr, h, w):
    ...
    results = [concat_images_list(
        *results[0:3]), concat_images_list(*results[3:7]), concat_images_list(*results[7:])]
    return results
...
class ShapeRenderMetrics(Loss):
    ...
    def __call__(self, data_pr, data_gt, step, **kwargs):
        rgb_gt = color_map_backward(
            data_pr['gt_rgb'].detach().cpu().numpy())  # h,w,3
        imgs = [rgb_gt]

        # compute psnr
        rgb_pr = color_map_backward(
            data_pr['ray_rgb'].detach().cpu().numpy())  # h,w,3
        ...
        # normal
        h, w, _ = rgb_pr.shape
        normal = color_map_backward(
            data_pr['normal'].detach().cpu().numpy())  # h,w,3
        ...
        imsave(f'{str(output_path)}/{step}-index-{data_index}.jpg',
               concat_images_list(*imgs, vert=True))
        return outputs


class NeRFMetrics(Loss):
    def __init__(self, cfg):
        pass
    ...
    def __call__(self, data_pr, data_gt, step, **kwargs):
        ...
        # depth
        h, w, _ = rgb_pr.shape
        imgs.append(process_key_img(data_pr['depth'], h, w))
...
class MaterialRenderMetrics(Loss):
    ...
    def __call__(self, data_pr, data_gt, step, *args, **kwargs):
        rgb_gt = color_map_backward(
            data_pr['rgb_gt'].detach().cpu().numpy())  # h,w,3
        rgb_pr = color_map_backward(
            data_pr['rgb_pr'].detach().cpu().numpy())  # h,w,3
        imgs = [rgb_gt, rgb_pr]
        ...
        output_imgs = [concat_images_list(
            *imgs[:5]), concat_images_list(*imgs[5:])]
        ...
        output_path.mkdir(exist_ok=True, parents=True)
        imsave(f'{str(output_path)}/{step}-index-{data_index}.jpg',
               concat_images_list(*output_imgs, vert=True))
        return outputs


name2metrics = {
    'nerf_render': NeRFMetrics,
